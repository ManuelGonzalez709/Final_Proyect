...
public class UserController implements Initializable {
    ...
    @FXML private TableColumn<User, String> nombUsuColumn;
        ...
        if(selectedUser != null){
            ...
            if (result.isPresent() && result.get() == ButtonType.OK) {
                try{
                    ...
                    if(me.borrarUsuario(params)) Util.mostrarDialogo("Server Message", "Borrado de Usuario","El usuario con id: "+params[0]+", ha sido borrado correctamente", Alert.AlertType.INFORMATION);
                    else Util.mostrarDialogo("Server Message", "Borrado de Usuario","El borrado no se pudo llevar a cabo :(", Alert.AlertType.INFORMATION);
                    CargarDatosTabla();
                }catch (Exception e){Util.mostrarDialogo("App Message", "Borrado de Usuario","Error en el Borrado "+e, Alert.AlertType.INFORMATION);}
            } else{
    ...
    @FXML private void btCambiarPass(){
        User selectedUser = tbUsuarios.getSelectionModel().getSelectedItem();
        ...
        if(selectedUser != null){
            try {
                ...
                Stage primaryStage = (Stage) tbUsuarios.getScene().getWindow(); // Reemplaza 'tbAnuncios' con cualquier nodo de tu ventana principal
    ...
    public void Actualizar(User usuario){
        try{
            String[]params = {Integer.toString(usuario.getId()),usuario.getNombre(),usuario.getApellidos(),usuario.getTelefono(),usuario.getEmail(),usuario.getFecha_nacimiento(),usuario.getNomb_usu()};
            if(!me.actualizarUsuario(params))Util.mostrarDialogo("App Message", "Actualizado de Usuario","Error en el Actualizado ", Alert.AlertType.INFORMATION);
            CargarDatosTabla();
        }catch (Exception e){Util.mostrarDialogo("App Message", "Actualizado de Usuario","Error en el Actualizado "+e, Alert.AlertType.INFORMATION);}
    }
}
